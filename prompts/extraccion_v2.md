...
**cuota_inicial_cop** (entero o null): el monto que el cliente dice tener para la cuota inicial de una compra financiada, en pesos. Convierte la jerga a pesos: "2 palos", "2 millones", "2 millonzitos" y "2000mil" son 2000000; "1,5 millones" es 1500000; "800mil" es 800000; "$1.200.000" es 1200000. Si el cliente dice una cifra de cero ("0 millones", "0 palos"), el valor es 0, no null. Si no dice monto, null.
- **Solo lleva monto cuando `manifesto_cuota_inicial` es `SI`.** Si es `NO` o `NO_INFORMA`, el valor es null, aunque el cliente haya dicho una cifra antes y luego se haya retractado.
- **Si `forma_pago` es `contado`, el valor es null.** El dinero para pagar de contado es el precio de la moto, no una cuota inicial.

**forma_pago**:
- `contado`: el cliente dice que paga de contado.
- `credito`: el cliente dice que la quiere a crédito, financiada o a cuotas. **Si el cliente habla de su cuota inicial sin decir la forma de pago, el valor es `credito`: solo se paga inicial cuando se financia.**
...
- **Si `forma_pago` es `contado`, el valor es `NO_INFORMA`.** Decir cuánto dinero tiene para pagar de contado no es manifestar una cuota inicial.
...
**pidio_cotizacion**:
- `SI`: el cliente pide o acepta que le envíen la cotización ("mándemela", "envíemela"). Pedir que le separen la moto no es pedir cotización.
...
- `precio`: el valor de la moto es el obstáculo. El cliente busca alternativas más baratas, pregunta por motos usadas, pide descuento o dice que el valor está fuera de su alcance.
- `tasa`: se queja del interés o de la tasa del crédito.
- `cuota`: la cuota mensual de la financiación le parece alta.
- `sin_inicial`: no tiene dinero para la cuota inicial, o la inicial que le piden es más de lo que tiene.
- `historial_crediticio`: su historial de crédito le puede impedir financiar (está reportado, en centrales de riesgo o en Datacrédito).
- `disponibilidad`: le preocupa la entrega o que no haya la moto.
- `tramites`: le preocupan los papeles o requisitos del proceso.
- `otra`: cualquier otra objeción, por ejemplo tener que consultarlo con alguien.
