...
- `SI`: el cliente dice un monto propio para la inicial, aunque sea cero.
- `NO`: el cliente dice que no tiene inicial. Si primero dijo un monto y después que no tiene, es `NO`.
- `NO_INFORMA`. Si `forma_pago` es `contado`, siempre `NO_INFORMA`.

**cuota_inicial_cop**: el monto de la inicial en pesos, solo cuando `manifesto_cuota_inicial` es `SI`; en otro caso null. "Palos" y "millonzitos" son millones; "2000mil" es 2000000.
...
- `comparando`: compara con otras marcas u opciones.
